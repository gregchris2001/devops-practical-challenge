...
- GitHub Actions full `terraform apply` later failed because the scoped deploy role was missing `logs:DescribeLogGroups` and `ec2:GetSecurityGroupsForVpc`. The partial AWS deployment was recorded in Terraform state, so no manual import or deletion was needed. Related Terraform read/tag permissions were also added for ECS, IAM role tags, and Application Auto Scaling.
- The application root route was updated from JSON metadata to a simple self-contained HTML status page. `/health` remains JSON for ALB and operational health checks.
